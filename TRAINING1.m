...
Q1 = zeros(3^9,9); % start with equal-random policy
actionMatrix = getActionsforStates(1);
...
N = zeros(1,m);
Q1_sum = zeros(1,m);
Q1_sum_rate = zeros(1,m);


%% Training Loop
for n = 1:m % play m games
    ...
    while (terminalState == 0)
        ...
        if(terminalState == 0) 
            % Q-Learner move
            [nextStateIndex,chosenAction] = epsilonGreedyAction(Q1(stateIndex,:),actionMatrix(stateIndex,:),epsilon);
            ...
            % learned sum
            if(terminalState == 0)
                learned = (reward + y*min(Q1(nextStateIndex,possibleActions)));
            else
                learned = reward;
            end
            % inertial sum
            inertia = Q1(stateIndex,chosenAction);
            % update Q-value
            Q1(stateIndex,chosenAction) = (1-a)*inertia + a*learned ;
        ...
        if (terminalState == 0)
            ...
            % learned sum
            if(terminalState == 0)
                learned = (reward + y*max(Q1(nextStateIndex,possibleActions)));
            else
                learned = reward;
            end
            % inertial sum
...
figure(1);
plot(N,abs(Q1_sum),'color', [0.5 0 1],'LineWidth',2)
title('Q1-Learning')
xlabel('Games')
ylabel('Abs Sum Q')
saveas(gcf,'Q1_sum.jpg')
...
saveas(gcf,'Q1_WLT.jpg')

figure(3);
plot(N,abs(Q1_sum_rate),'color', [0.5 0 1],'LineWidth',2)
title('Q1-Learning Rate')
xlabel('Games')
ylabel('Avg Sum Q Diffrence')
saveas(gcf,'Q1_sum_rate.jpg')
